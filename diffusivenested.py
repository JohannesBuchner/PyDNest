...
class DiffusiveSampler(object):
	...
	def __next__(self):
		for i in range(self.saveInterval):
			...
			if len(self.levels) < self.maxLevels and self.levels[-1].cutoff < Li:
				self.keep.append(Li)
			...
			if len(self.keep) > self.newLevelInterval:
				...
				cutoff = self.keep[ii]
				print "# Creating level %d with logL = %.5f." % (len(self.levels), cutoff)
...
if __name__ == '__main__':
	...
	sampler = DiffusiveSampler(nlive_points = 1,
		priortransform=priortransform, loglikelihood=loglikelihood, 
		draw_constrained = constrainer, ndim=1)
	print 'running sampler'
	result = diffusive_integrator(tolerance=0.01, sampler=sampler)
